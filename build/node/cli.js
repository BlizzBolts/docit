...
import { isFileExists, readUserConfigFile, resolveAbsPath, } from './utils/index.js';
const argv = minimist(process.argv.slice(2));
const command = argv._[0];
const root = argv._[1] || 'docs';
const pkg = fsx.readJSONSync(PKG_JSON_PATH);
console.log(chalk.greenBright(`Running Docit@${pkg.version}`));
const configFilePath = path.resolve(resolveAbsPath(root), './.docit/docit.config.js');
let server = null;
const bootstrap = () => {
